...
def select_hero_names_and_squad_names_of_heroes_belonging_to_a_team():
    ...
    INNER JOIN squads ON superheroes.squad_id = squads.id;
    """

def reformatted_query():
    return """SELECT superheroes.name, squads.name as team FROM superheroes
    INNER JOIN squads ON superheroes.squad_id = squads.id ORDER BY team;"""
...
def all_squads():
    return """SELECT squads.name, COUNT(superheroes.id) as num_of_members FROM
    squads LEFT JOIN superheroes ON squads.id = superheroes.squad_id GROUP BY
    squads.name;"""
